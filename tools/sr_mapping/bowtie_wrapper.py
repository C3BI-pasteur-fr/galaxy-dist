...
def __main__():
    ...
    parser.add_option( '', '--output_log', dest='output_log', help='The output log file')
    ...
    if options.genomeSource == 'history' and not options.do_not_build_index:
        ...
        try:
            ...
            stdlog = open(options.output_log, 'ab')
            stderr = ''
            buffsize = 1048576
            try:
                while True:
                    stdlog.write(tmp_stderr.read( buffsize ))
