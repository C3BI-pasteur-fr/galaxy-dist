...
class LocalJobRunner( BaseJobRunner ):
    ...
    def queue_job( self, job_wrapper ):
        ...
        try:
            ...
            proc = subprocess.Popen( args=command_line,
                                     ...
                                     stderr=stderr_file,
                                     env=self._environ,
                                     preexec_fn=os.setpgrp, close_fds = True ) #PASTEUR PATCH MAREUIL
    ...
    def _terminate( self, proc ):
        ...
        for i in range(60):				#PASTEUR PATCH MAREUIL
            if proc.poll() is not None:			#PASTEUR PATCH MAREUIL
                return proc.wait()			#PASTEUR PATCH MAREUIL
            else:					#PASTEUR PATCH MAREUIL
                sleep( 1 )				#PASTEUR PATCH MAREUIL
        #sleep( 1 )					#PASTEUR PATCH MAREUIL
        if proc.poll() is None:
            log.warning(  "stop_job(): %d: refuse to die after kill -15 patch Fabien Mareuil no kill -9" % proc.pid )   #PASTEUR PATCH MAREUIL
