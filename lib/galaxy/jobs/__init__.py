...
#PASTEUR PATCH MAREUIL
        if self.app.config.use_modules:
            path_module_conf_sample = os.path.dirname(self.command_line.split()[1]) + "/module_conf.xml.sample"
            log.debug('MODULE DEBUG : Path module_conf.xml.sample %s' % ( path_module_conf_sample ))
            self.dependency_module = self.module_dependency( job.tool_id, job.tool_version, path_module_conf_sample )
            if self.dependency_module:
                dict_module = {'source_module':self.app.config.use_modules, 'modules': self.dependency_module[0]}
                script = module_template % ( dict_module )
                module_path = os.path.join( self.working_directory, "module.sh" )
                modulefile = file( module_path, "w" )
                modulefile.write( script )
                modulefile.close()
                os.chmod( module_path, 0750 )
                self.dependency_module_commands = [". " + module_path]
            else:
                self.dependency_module_commands = None
        else:
            self.dependency_module_commands = None
#PASTEUR PATCH MAREUIL
...
#PASTEUR PATCH MAREUIL
    def module_dependency( self, job_tool_id, job_tool_version, module_sample_path ):
