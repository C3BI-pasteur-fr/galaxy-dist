        ...
        if jiha:
            try:
                ...
                for dataset_attrs in datasets_attrs:
                    ...
                    ## datasets_usage_counts[ temp_dataset_file_name ] += 1          # PASTEUR_PATCH Sophie Creno
                    if ( not dataset_attrs['purged'] ):				     # PASTEUR_PATCH Sophie Creno
                        datasets_usage_counts[ temp_dataset_file_name ] += 1         # PASTEUR_PATCH Sophie Creno  
                ...
                for dataset_attrs in datasets_attrs:
                    ...
                    hda.deleted = dataset_attrs['deleted']            # PASTEUR_PATCH Sophie Creno
                    hda.purged = dataset_attrs['purged']  	      # PASTEUR_PATCH Sophie Creno
...
#                        shutil.copyfile( temp_dataset_file_name, hda.file_name )
                    ...
                    if not file_in_dir( temp_dataset_file_name, os.path.join( archive_dir, "datasets" )) \
				and not dataset_attrs['purged'] :
                            				 	                    						# PASTEUR_PATCH Sophie Creno
                        raise Exception( "Invalid dataset path: %s" % temp_dataset_file_name )    				# PASTEUR_PATCH Sophie Creno
                    if not dataset_attrs['purged'] :            		  						# PASTEUR_PATCH Sophie Creno
                        if datasets_usage_counts[ temp_dataset_file_name ] == 1:  						# PASTEUR_PATCH Sophie Creno
                            shutil.move( temp_dataset_file_name, hda.file_name )  						# PASTEUR_PATCH Sophie Creno 
                            hda.set_total_size()	 			  						# PASTEUR_PATCH Sophie Creno
                        else:							  						# PASTEUR_PATCH Sophie Creno
                            datasets_usage_counts[ temp_dataset_file_name ] -= 1  						# PASTEUR_PATCH Sophie Creno
                            shutil.copyfile( temp_dataset_file_name, hda.file_name ) 						# PASTEUR_PATCH Sophie Creno
...
class JobExportHistoryArchiveWrapper( object, UsesHistoryMixin, UsesAnnotations ):
    ...
    def setup_job( self, trans, jeha, include_hidden=False, include_deleted=False ):
            ...
            def default( self, obj ):
                ...
                if isinstance( obj, trans.app.model.HistoryDatasetAssociation ):
                    return {
                        ...
                        "deleted": obj.deleted,
                        "purged": obj.visible,     # PASTEUR_PATCH Sophie Creno
			"visible": obj.visible,
        ...
        #datasets = self.get_history_datasets( trans, history ) 	# PASTEUR_PATCH Sophie Creno 
        datasets = self.get_history_datasets( trans, history, show_deleted=True, \
                                              show_hidden=True, show_purged=True )    # PASTEUR_PATCH Sophie Creno
	included_datasets = []
        datasets_attrs = []
